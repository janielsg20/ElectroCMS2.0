...
## Por qué existe
Esta microfase existe para **Target compilers boundary** dentro de la responsabilidad de `F13`: resolver workflows/automation/providers con Rete. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Target compilers boundary** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Web/React/Native can use Rete engine runtime.
2. LAMP/WordPress compile portable action semantics to PHP/WP handlers.
3. Do not embed Rete editor packages in generated runtime.
4. Only include rete-engine/history if target runtime needs it.
5. GraphQL/custom code remain future adapters.
