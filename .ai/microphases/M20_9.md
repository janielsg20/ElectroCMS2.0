...
## Por qué existe
Esta microfase existe para **Native source smoke test** dentro de la responsabilidad de `F20`: generar runtime Expo/data/router/admin. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Native source smoke test** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Generate representative app.
2. Install deps.
3. Typecheck/Expo config validation.
4. Prebuild/build smoke where environment supports.
5. Verify no editor packages.
