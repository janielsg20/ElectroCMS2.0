...
**Fase propietaria:** F01 — Monorepo, boundaries y CI

## Por qué existe
Esta microfase existe para **Configurar lint, test y build** dentro de la responsabilidad de `F01`: crear boundaries, tooling y CI. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Configurar lint, test y build** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Configura ESLint/formatter sin reglas contradictorias.
2. Configura Vitest para unit/contract/integration.
3. Configura Playwright para Studio E2E pero no escribas aún tests funcionales extensos.
4. Crea scripts root `lint`, `typecheck`, `test`, `test:e2e`, `build`, `check`.
5. Prueba todos los scripts en un repo vacío funcional.
