...
## Por qué existe
Esta microfase existe para **Native data/query adapters** dentro de la responsabilidad de `F20`: generar runtime Expo/data/router/admin. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Native data/query adapters** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Record repositories implement same application interfaces.
2. Query field paths map to SQLite JSON extraction.
3. React Query Builder output reused where compatible.
4. Transactions.
