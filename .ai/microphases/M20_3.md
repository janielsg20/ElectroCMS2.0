...
## Por qué existe
Esta microfase existe para **Expo SQLite + Drizzle stable schema** dentro de la responsabilidad de `F20`: generar runtime Expo/data/router/admin. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Expo SQLite + Drizzle stable schema** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Define same conceptual generic tables for Native.
2. JSON text for record data.
3. Relations/taxonomies stable tables.
4. Bundle Drizzle migrations.
5. No new table per Content Type.
