...
## Por qué existe
Esta microfase existe para **Trazar Prompt Maestro 1.0 al plan 2.0** dentro de la responsabilidad de `F00`: validar engines, ownership y POCs antes de construir producto. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Trazar Prompt Maestro 1.0 al plan 2.0** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
## Precondiciones
- Gate de entrada de `F00` satisfecho.
...
1. Lee todas las secciones funcionales del Prompt Maestro 1.0.
2. Registra cada requisito en una tabla con: requisito, comportamiento observable, módulo propietario, fase, destinos.
3. Marca como `preservado`, `simplificado mediante engine`, `convertido a preset/composite` o `diferido`.
4. No aceptes un requisito sin dueño.
5. Genera `TRACEABILITY_MATRIX.md` y úsala como gate de F00.
