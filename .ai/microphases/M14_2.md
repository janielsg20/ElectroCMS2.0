...
## Por qué existe
Esta microfase existe para **RHF runtime y Zod compiler** dentro de la responsabilidad de `F14`: resolver Forms con RHF/Zod/ActionGraph. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **RHF runtime y Zod compiler** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. RHF maneja values/dirty/touched/submit.
2. Compila canonical validation rules a Zod.
3. Async/external validation queda en adapter separado.
4. Errors usan accessible message.
5. No duplicar validation engine.
