...
## Por qué existe
Esta microfase existe para **Auth/RBAC/admin** dentro de la responsabilidad de `F24`: generar LAMP. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Auth/RBAC/admin** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Generate secure session/auth.
2. Password hashing.
3. Permission evaluator equivalent.
4. Admin screens from AdminScreen config.
5. Field/resource checks server-side.
