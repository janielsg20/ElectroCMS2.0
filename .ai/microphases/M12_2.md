...
## Por qué existe
Esta microfase existe para **Permission evaluator** dentro de la responsabilidad de `F12`: resolver auth/permissions/audit. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Permission evaluator** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Central ElectroPermissionEvaluator.
2. Checks resource/action/field/route.
3. Service layer enforces, not UI only.
4. Reason returned for diagnostics.
5. Test direct unauthorized call.
