...
**Fase propietaria:** F06 — Editor avanzado, responsive y portabilidad visual

## Por qué existe
Esta microfase existe para **Multi-select, Group/Ungroup y Resize mediante extensión fina de Puck** dentro de la responsabilidad de `F06`: añadir capacidades avanzadas/responsive sobre Puck. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Multi-select, Group/Ungroup y Resize mediante extensión fina de Puck** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Primero evalúa APIs actuales Puck y extensión segura.
2. Implementa multi-select como Electro overlay solo si no rompe single selection internals.
3. Group crea Container preset y reparent seguro.
4. Resize solo para definitions resizable.
5. Si una función exige fork profundo de Puck, marca `deferred` y documenta alternativa; no forkar por defecto.
