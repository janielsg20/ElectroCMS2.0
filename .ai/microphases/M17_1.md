...
## Por qué existe
Esta microfase existe para **Frontend runtime preview** dentro de la responsabilidad de `F17`: previsualizar/diagnosticar runtime. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Frontend runtime preview** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
## Precondiciones
- Gate de entrada de `F17` satisfecho.
...
1. Renderiza ElectroDocument usando runtime-web, no Puck editor chrome.
2. Usa data/binding/query/state/action adapters reales.
3. Route context real.
4. Loading/empty/error.
5. No mutation por side effect de render.
