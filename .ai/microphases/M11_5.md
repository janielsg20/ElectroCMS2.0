...
## Por qué existe
Esta microfase existe para **State panel/debug** dentro de la responsabilidad de `F11`: resolver runtime state con Zustand. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **State panel/debug** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Definition list Name/Scope/Type/Default/Persist/Usage.
2. Debug current values separate.
3. Delete detects usages.
4. Mobile sheet.
5. Do not mix Studio workspace state.
