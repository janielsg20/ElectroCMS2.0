...
## Por qué existe
Esta microfase existe para **Platform overrides y diagnostics** dentro de la responsabilidad de `F06`: añadir capacidades avanzadas/responsive sobre Puck. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Platform overrides y diagnostics** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Context Web/Android/iOS.
2. Override platform solo cuando propiedad lo necesita.
3. Badge Web-only/Native adapted.
4. Consume las declaraciones de capability/version del registry contract definido en domain para mostrar badges básicos; no construyas todavía traversal/analyzer global.
5. El bloqueo completo de export pertenece al Compatibility Analyzer posterior; esta microfase solo edita overrides y muestra la capability declarada por el componente/propiedad.
