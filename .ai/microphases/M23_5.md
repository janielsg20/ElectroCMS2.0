...
## Por qué existe
Esta microfase existe para **Capacitor fallback exporter** dentro de la responsabilidad de `F23`: generar Android/iOS/Capacitor. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Capacitor fallback exporter** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Use React/Web output.
2. Only user choice or compatibility recommendation.
3. Label WebView/hybrid clearly.
4. Map supported native plugins separately.
5. Do not auto-switch silently.
