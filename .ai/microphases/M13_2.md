...
## Por qué existe
Esta microfase existe para **Rete editor + history plugin** dentro de la responsabilidad de `F13`: resolver workflows/automation/providers con Rete. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Rete editor + history plugin** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Palette Trigger/Logic/Data/UI/Auth/API/Native.
2. Rete canvas center/inspector right.
3. Use rete-history-plugin for add/remove/move/connect.
4. Wire undo/redo to Rete history.
5. No custom graph history.
