...
## Por qué existe
Esta microfase existe para **Local project package** dentro de la responsabilidad de `F22`: generar outputs Web/Local con dependencias mínimas. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Local project package** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Manifest + canonical snapshot + content data + media + versions.
2. Checksums/safe paths.
3. Reimport path.
4. Recovery metadata.
5. Round-trip test.
