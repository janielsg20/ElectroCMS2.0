...
## Por qué existe
Esta microfase existe para **Asset localization and generated docs** dentro de la responsabilidad de `F22`: generar outputs Web/Local con dependencias mínimas. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Asset localization and generated docs** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Collect referenced assets only.
2. Stable paths/checksum dedupe.
3. Generate README per target.
4. No secret values.
5. Output filename deterministic except allowed version/timestamp.
