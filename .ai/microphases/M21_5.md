...
## Por qué existe
Esta microfase existe para **Contacts, Calendar y Biometrics** dentro de la responsabilidad de `F21`: mapear device capabilities. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Contacts, Calendar y Biometrics** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Optional adapters only when used.
2. Minimal requested data.
3. Biometrics validates hardware/enrollment.
4. Fallback auth path.
5. No mass address-book import by default.
