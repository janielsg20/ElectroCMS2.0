...
## Por qué existe
Esta microfase existe para **POC Native runtime** dentro de la responsabilidad de `F00`: validar engines, ownership y POCs antes de construir producto. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **POC Native runtime** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Genera Expo project mínimo con stable Expo Router Stack.
2. Añade standard JS Tabs solo en una route group de prueba; no uses unstable-native-tabs.
3. Usa Expo SQLite + Drizzle con content_records, relations y field index genéricos.
4. Renderiza Container/Text/Button/List.
5. Añade Zustand persisted state.
6. Integra Refine Core headless mínimo con custom Native List/Create flow sobre ElectroDataProvider; no TanStack DOM table.
7. Ejecuta deep link/route guard test.
8. Genera permission-free app primero; añade una native capability para comprobar config pruning.
9. Registra source/build blockers Android/iOS sin inventar artifacts.
