...
## Por qué existe
Esta microfase existe para **Blueprint group: Commerce y operaciones** dentro de la responsabilidad de `F16`: instalar Project Kits/Blueprints sin engines nuevos. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Blueprint group: Commerce y operaciones** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Implementa Store, Inventory, Restaurant, Marketplace y Vehicles según BLUEPRINT_MATRIX.
2. Usa core components + commerce blocks, no nuevos engines.
3. Backend screens from Refine config.
4. Queries/forms/actions reales.
