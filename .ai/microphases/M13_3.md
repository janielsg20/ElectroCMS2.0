...
**Fase propietaria:** F13 — Action Flow, Automation y API Providers con Rete

## Por qué existe
Esta microfase existe para **Rete ControlFlow/Dataflow runtime JS** dentro de la responsabilidad de `F13`: resolver workflows/automation/providers con Rete. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Rete ControlFlow/Dataflow runtime JS** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Usa `rete-engine` ControlFlowEngine/DataflowEngine como runtime JS del graph cuando la semántica del graph sea compatible.
2. El adapter construye nodes/runtime objects a partir de ElectroActionGraph; esos objects nunca se persisten.
3. No generes un segundo `ExecutionPlan` genérico para Web/React/Native.
4. Effects Data/State/Navigation/UI/Auth/API/Native se ejecutan mediante ports/adapters Electro inyectados en el node runtime.
5. Antes de ejecutar, valida required inputs, connections, target capabilities y permissions.
6. Usa correlationId/cancellation context para tracing/automation.
7. PHP/WordPress no cargan Rete JS: sus exporters compilan ActionGraph semantics a handlers target-native.
8. Prueba branch Condition, Dataflow value, cancellation y error propagation.
