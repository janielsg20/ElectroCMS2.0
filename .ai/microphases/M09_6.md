...
## Por qué existe
Esta microfase existe para **Filter component único, facets y apply modes** dentro de la responsabilidad de `F09`: resolver bindings/queries/listings/facets/cache. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Filter component único, facets y apply modes** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Mantén un único component `Filter` con `filterType`; Search/Select/Range/Checkbox/Radio/Date/Taxonomy/Sort son aliases.
2. Añade `applyMode`: `live`, `manual`, `url`, `reload` según runtime/target.
3. `FilterState` es runtime state; no modifica la QueryDefinition guardada.
4. Soporta applied chips, clear-one, clear-all y Reset action.
5. Para `faceted`, calcula counts mediante GenericFieldIndexer; no ejecutes una query completa por cada option si existe índice.
6. Soporta hierarchy para taxonomy filters.
7. Pagination/Load More consume el mismo FilterState.
8. LAMP/WordPress exporter decide AJAX vs full reload según target profile; no crea un filter engine diferente.
