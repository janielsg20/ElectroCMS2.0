...
## Por qué existe
Esta microfase existe para **Advanced editor QA** dentro de la responsabilidad de `F06`: añadir capacidades avanzadas/responsive sobre Puck. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Advanced editor QA** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Responsive inherit/reset.
2. Platform override.
3. Guides/snapping.
4. Context actions.
5. Accessibility keyboard alternative.
6. Performance de documento mediano.
7. Lista los archivos existentes afectados antes de crear uno nuevo.
8. Define input y output tipados de la capacidad.
9. Separa persistent state de transient UI/engine state.
10. Implementa error/empty/disabled states aplicables.
11. Si hay UI, usa primitives ElectroCMS y no componentes nativos sin estilo.
12. Si hay mutación persistente, pasa por el service/adapter propietario; nunca `UI -> raw database`.
13. Añade tests antes de marcar el resultado terminado.
