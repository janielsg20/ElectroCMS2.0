...
**Fase propietaria:** F25 — WordPress Theme + Companion Plugin Exporter

## Por qué existe
Esta microfase existe para **Roles/Admin/Actions** dentro de la responsabilidad de `F25`: generar WordPress. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Roles/Admin/Actions** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Roles/capabilities map to WP APIs.
2. AdminScreen generated with capability checks.
3. ActionGraph semantics compile to WP/PHP handlers.
4. REST endpoints only when needed.
5. Nonces/permissions all mutations.
