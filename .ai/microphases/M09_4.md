...
## Por qué existe
Esta microfase existe para **Relations/Taxonomy query semantics** dentro de la responsabilidad de `F09`: resolver bindings/queries/listings/facets/cache. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Relations/Taxonomy query semantics** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Añade query clauses portables para relation/taxonomy context.
2. Implementa target adapter PGlite.
3. Query UI muestra relation/taxonomy controls por field type.
4. IDs canónicos, no labels.
5. Preview real.
