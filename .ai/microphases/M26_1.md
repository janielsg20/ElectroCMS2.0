...
## Por qué existe
Esta microfase existe para **Security hardening fail-closed** dentro de la responsabilidad de `F26`: endurecer y proteger boundaries post-core. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Security hardening fail-closed** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
## Precondiciones
- Gate de entrada de `F26` satisfecho.
...
1. Validate imported manifest, project object wrappers, schema versions and checksums before any write.
2. Validate ZIP/package paths against zip-slip/path traversal.
3. Validate filename/MIME/content/size for media; sanitize SVG and dangerous HTML.
4. Query adapters must use parameterized values and fail closed on unsupported formatter rules.
5. Review Auth/Permissions at UI AND application/server boundary.
6. Redact secrets in Debug, Audit, Action Trace and generated docs.
7. Run dependency vulnerability/license audit on Studio and generated fixtures.
8. Review optional HTML/Embed capability and CSP/sandbox strategy by target.
9. No arbitrary JS custom code in core.
10. Block release on unresolved critical/high findings.
