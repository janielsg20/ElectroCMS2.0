...
## Por qué existe
Esta microfase existe para **Listing runtime para Query y Collection** dentro de la responsabilidad de `F09`: resolver bindings/queries/listings/facets/cache. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Listing runtime para Query y Collection** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Listing referencia queryId + item template/subtree.
2. El visible item Repeater se implementa como Listing `sourceType=collection-binding`; no existe un segundo repeater runtime.
3. Loading/empty/error/pagination.
4. Virtualiza listas largas donde corresponda.
5. No crear QueryResult/RelatedContent widgets separados.
