...
## Por qué existe
Esta microfase existe para **Fallback registry** dentro de la responsabilidad de `F18`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
1. Explicit transformations only.
2. Examples Container Grid->Wrap, Web Embed->Capacitor recommendation.
3. Fallback preserves intent or emits warning.
4. Never silently drop node/action.
5. Version fallback rule.
