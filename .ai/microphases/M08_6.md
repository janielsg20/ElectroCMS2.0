...
## Por qué existe
Esta microfase existe para **Generic Typed Field Indexer** dentro de la responsabilidad de `F08`: modelar CMS/workflow/indexes sin DDL dinámico. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Generic Typed Field Indexer** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Sustituye el concepto vago de `index hints` por un `GenericFieldIndexer` explícito.
2. Solo fields con flags `searchable`, `filterable`, `sortable` o `faceted` entran en `record_field_index`.
3. Cada index row guarda projectId, recordId, contentTypeId, fieldId, valueType y una de text/number/date/bool/normalizedText + ordinal cuando un repeater lo necesite.
4. Record create/update/delete actualiza records + index rows dentro de una misma transacción.
5. No crear columnas/tables por field y no crear expression index por defecto.
6. Query adapter usa `record_field_index` para sort/facets/frequent filters y JSON extraction como fallback para fields no indexados.
7. Añade rebuild/reindex operation por Content Type cuando cambie una definición indexable.
8. Mide el costo antes de introducir índices físicos adicionales.
