...
## Por qué existe
Esta microfase existe para **Capability model y registry** dentro de la responsabilidad de `F18`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
## Precondiciones
- Gate de entrada de `F18` satisfecho.
...
1. Targets Web/React/Static/PWA/Android/iOS/Capacitor/LAMP/WordPress.
2. Status native/adapted/fallback/unsupported/planned.
3. Registry by component/style/binding/action/provider.
4. Missing declaration is not silently supported.
5. Version registry.
