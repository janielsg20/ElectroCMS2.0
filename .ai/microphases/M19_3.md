...
## Por qué existe
Esta microfase existe para **Native core content** dentro de la responsabilidad de `F19`: renderizar Native. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Native core content** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Text/Heading -> Text.
2. Image/Gallery -> native asset/image components.
3. Button/Link -> Pressable/navigation.
4. Video/Audio/Carousel via target adapters.
5. Embed/Iframe unsupported/adapted via WebView only when explicit.
