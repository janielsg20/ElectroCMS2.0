...
## Por qué existe
Esta microfase existe para **Routes/renderers/forms** dentro de la responsabilidad de `F24`: generar LAMP. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Routes/renderers/forms** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Compile ElectroRoutes/documents/templates.
2. Render bindings/query/listings.
3. Forms validate and run server action handlers.
4. CSRF.
5. Escape output.
