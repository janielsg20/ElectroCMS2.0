...
## Por qué existe
Esta microfase existe para **POC Studio DB genérica** dentro de la responsabilidad de `F00`: validar engines, ownership y POCs antes de construir producto. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **POC Studio DB genérica** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Crea PGlite en el official multi-tab Worker setup, no main-thread singleton.
2. Define Drizzle schema mínimo con `projects`, `project_objects`, `project_revisions`, `content_records`, `relation_edges` y `record_field_index`.
3. Guarda dos Project Objects distintos y modifica solo uno; verifica que el otro row/checksum no cambie.
4. Crea dos Content Types lógicos y records sin crear tablas físicas nuevas.
5. Marca un field como faceted y genera index rows en record_field_index.
6. Añade field al ElectroSchema y confirma cero ALTER TABLE.
7. Abre dos worker clients/tabs y verifica consistencia.
8. Cierra/reabre y verifica project objects, records e index.
9. Mide latencia base para save object/query.
