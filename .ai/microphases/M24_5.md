...
## Por qué existe
Esta microfase existe para **Action/query target adapters** dentro de la responsabilidad de `F24`: generar LAMP. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Action/query target adapters** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Compile ElectroQuery to MySQL-safe parameterized conditions.
2. Compile ActionGraph node semantics to PHP handlers.
3. HTTP provider server requests.
4. Unsupported native actions diagnostic.
5. No JS workflow engine assumption on server.
