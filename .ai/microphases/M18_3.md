...
**Fase propietaria:** F18 — Platform Capability Registry y Compatibility Analyzer

## Por qué existe
Esta microfase existe para **Binding/query/form/action capabilities** dentro de la responsabilidad de `F18`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Binding/query/form/action capabilities** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Bindings verify source availability.
2. Queries verify runtime/compiler support.
3. Forms verify field/action target.
4. Action nodes verify target/native permission/server compiler.
5. Provider requirements included.
