...
## Por qué existe
Esta microfase existe para **Android exporter y build** dentro de la responsabilidad de `F23`: generar Android/iOS/Capacitor. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Android exporter y build** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Generate Expo Android source.
2. Prebuild if configured.
3. APK debug when SDK exists.
4. AAB only with release signing.
5. Verify artifact path before reporting.
