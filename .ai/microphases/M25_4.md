...
## Por qué existe
Esta microfase existe para **Forms/Queries/Filters** dentro de la responsabilidad de `F25`: generar WordPress. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Forms/Queries/Filters** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Compile Query semantics to WP_Query/meta_query/tax_query or safe custom SQL when required.
2. Forms use nonces/sanitize/validate.
3. Filters preserve state/pagination.
4. Uploads use WP media APIs.
5. No JetFormBuilder dependency.
