...
## Por qué existe
Esta microfase existe para **Location y Camera/Media** dentro de la responsabilidad de `F21`: mapear device capabilities. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Location y Camera/Media** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Location returns canonical coordinate result.
2. Camera capture feeds Media pipeline.
3. Contextual permission request.
4. Cancel is not error.
5. No background location unless future explicit feature.
