...
## Por qué existe
Esta microfase existe para **Native state/auth restore** dentro de la responsabilidad de `F20`: generar runtime Expo/data/router/admin. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Native state/auth restore** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Initialize DB migrations first.
2. Restore Zustand persisted state.
3. Restore auth/session using secure adapter when needed.
4. Only then mount protected routes.
5. Error recovery screen on bootstrap failure.
