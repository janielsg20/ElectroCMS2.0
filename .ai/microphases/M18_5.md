...
## Por qué existe
Esta microfase existe para **Compatibility Analyzer** dentro de la responsabilidad de `F18`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Compatibility Analyzer** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Traverse project snapshot, documents, styles, bindings, queries, forms, actions, providers, media.
2. Aggregate diagnostics by target.
3. Does not mutate project.
4. Diagnostic includes source location and fix.
5. Cache by project revision/target.
