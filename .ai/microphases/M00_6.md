...
## Por qué existe
Esta microfase existe para **POC Action Flow Rete** dentro de la responsabilidad de `F00`: validar engines, ownership y POCs antes de construir producto. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **POC Action Flow Rete** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Crea ElectroActionGraph mínimo Trigger -> Condition -> Data/Toast.
2. Mapéalo al editor Rete.
3. Ejecuta con rete-engine ControlFlow/Dataflow.
4. Activa rete-history-plugin y prueba undo/redo de node/connection.
5. Verifica que ElectroActionGraph no persista clases Rete.
