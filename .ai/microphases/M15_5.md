...
## Por qué existe
Esta microfase existe para **Dashboard, Metric, Chart, Saved Views** dentro de la responsabilidad de `F15`: resolver Backend con Refine/TanStack. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
1. Dashboard es un ElectroDocument kind=backend-screen compuesto visualmente; no crea un dashboard runtime separado.
...
5. Native chart renderer se implementa en F19/F20 mediante Victory Native solo cuando dependency compatibility se verifique y Chart esté usado.
