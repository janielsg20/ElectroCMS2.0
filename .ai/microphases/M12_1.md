...
## Por qué existe
Esta microfase existe para **ElectroRole/Permission model** dentro de la responsabilidad de `F12`: resolver auth/permissions/audit. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **ElectroRole/Permission model** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Define `ElectroRole` y PermissionPolicy por capability, no por nombres hardcoded.
...
3. Resource rules pueden aplicar a ContentType/resource IDs concretos.
