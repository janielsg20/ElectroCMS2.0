...
## Por qué existe
Esta microfase existe para **Record CRUD y validation** dentro de la responsabilidad de `F08`: modelar CMS/workflow/indexes sin DDL dinámico. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Record CRUD y validation** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. content_records almacena data JSON.
2. Antes de write, compila/ejecuta validation desde ElectroSchema.
3. Soft delete por status/deletedAt policy.
4. UI Content usa DataView desktop/cards mobile.
5. No insertar datos saltándose service.
