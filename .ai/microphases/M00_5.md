...
## Por qué existe
Esta microfase existe para **POC Query portable** dentro de la responsabilidad de `F00`: validar engines, ownership y POCs antes de construir producto. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **POC Query portable** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Crea ElectroQueryDefinition con wrapper RQB versionado.
2. Añade rules nested AND/OR sobre fields de content_records.
3. Mapea indexed field a record_field_index y otro field a JSON extraction.
4. Usa RQB formatter parameterizado/Drizzle compatible where possible.
5. Inyecta una rule/operator deliberadamente unsupported y verifica que Electro produce blocker; NO debe aceptar fallback true/no-op.
6. Crea facet count sobre el indexer.
7. Crea una query multi-source simple y valida output-shape compatibility.
8. Prueba injection payload y comprueba parámetros separados.
9. Documenta exactamente qué parte resuelve RQB y qué parte resuelve Electro adapter.
