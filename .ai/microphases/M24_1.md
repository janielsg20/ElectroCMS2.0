...
**Fase propietaria:** F24 — LAMP Exporter

## Por qué existe
Esta microfase existe para **LAMP project structure** dentro de la responsabilidad de `F24`: generar LAMP. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **LAMP project structure** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
## Precondiciones
- Gate de entrada de `F24` satisfecho.
...
1. Generate public/index/router, app controllers/services/repositories, config, migrations, assets.
2. Separate templates from business logic.
3. Use Composer only if justified.
4. README install.
5. No demo-only backend.
