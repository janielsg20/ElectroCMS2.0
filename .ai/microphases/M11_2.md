...
## Por qué existe
Esta microfase existe para **Zustand store adapter** dentro de la responsabilidad de `F11`: resolver runtime state con Zustand. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Zustand store adapter** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Generate/instantiate vanilla Zustand stores from definitions.
2. React hooks use selectors.
3. Component/page stores scoped correctly.
4. No duplicate Redux/custom event store.
