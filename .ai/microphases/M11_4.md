...
## Por qué existe
Esta microfase existe para **State Bindings y Actions** dentro de la responsabilidad de `F11`: resolver runtime state con Zustand. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **State Bindings y Actions** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Binding resolver reads state.
2. Action nodes SetState/ResetState use adapter.
3. Type compatibility.
4. Derived values use selectors.
5. No arbitrary mutation outside store actions.
