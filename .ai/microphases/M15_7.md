...
## Por qué existe
Esta microfase existe para **Backend visual composition** dentro de la responsabilidad de `F15`: resolver Backend con Refine/TanStack. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Backend visual composition** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Puck Admin palette renders DataView/Metric/Chart/Kanban/Calendar Core plus RecordForm as Form preset, AdminNavigation as Navigation preset and RecordDetail as Container/Binding block.
2. Same Puck history for backend document editing.
3. Refine handles runtime CRUD only.
4. Theme backend separate.
5. Role-specific navigation config.
