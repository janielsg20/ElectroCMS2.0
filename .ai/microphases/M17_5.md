...
## Por qué existe
Esta microfase existe para **Query/Action/State traces** dentro de la responsabilidad de `F17`: previsualizar/diagnosticar runtime. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Query/Action/State traces** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Query trace: queryId, params safe summary, duration, count.
2. Action trace: node timeline/status/duration.
3. State: current value/scope/usage.
4. Click item navigates to builder.
5. Trace storage bounded.
