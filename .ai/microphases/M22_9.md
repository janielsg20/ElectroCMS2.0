...
## Por qué existe
Esta microfase existe para **Web export build fixtures** dentro de la responsabilidad de `F22`: generar outputs Web/Local con dependencias mínimas. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Web export build fixtures** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Same snapshot exported twice -> deterministic comparison.
2. React install/typecheck/build.
3. Static smoke.
4. PWA offline/install smoke.
5. Local reimport.
