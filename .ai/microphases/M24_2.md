...
## Por qué existe
Esta microfase existe para **MySQL content store/compiler** dentro de la responsabilidad de `F24`: generar LAMP. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **MySQL content store/compiler** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
1. Initial target uses generic records JSON or optimized target mapping selected by exporter strategy.
2. Relations/taxonomies explicit.
3. Prepared PDO.
4. Migrations versioned.
5. No Postgres-only syntax.
