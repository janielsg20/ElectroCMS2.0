...
## Por qué existe
Esta microfase existe para **AuthProvider Port y local runtime** dentro de la responsabilidad de `F12`: resolver auth/permissions/audit. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **AuthProvider Port y local runtime** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
## Ubicación visual
Administrar > Roles y Debug > Permission Simulator.
...
1. Define login/logout/register/currentUser/session.
2. Use hashed passwords only in local/server runtime where appropriate.
3. Native secure session adapter later.
4. Do not bind to external SaaS.
5. Separate auth from permission.
